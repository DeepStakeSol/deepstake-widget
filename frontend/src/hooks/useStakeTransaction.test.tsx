...
describe('useStakeTransaction', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    generateKeyPairSignerMock.mockResolvedValue({
      address: 'new-stake-account',
      keyPair: 'keypair',
    })
  ...
  it('runs the successful stake transaction flow', async () => {
    ...
    expect(confirmTransactionMock).toHaveBeenCalledWith('devnet', {
      ...
      timeout: 30000,
      interval: 1000,
      cacheMutation: { walletAddress: 'wallet-address', mutation: 'native-stake' },
    })
  ...
  it('stores errors without refreshing mutation data', async () => {
